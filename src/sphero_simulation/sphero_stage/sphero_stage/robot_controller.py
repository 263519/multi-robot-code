...
class RobotController(Node):
    def __init__(self):
        ...
        for i in range(NUM_ROBOTS):
            ...
            sub = self.create_subscription(Odometry, sub_topic,
    ...
    def compute_flocking(self):
        ...
        for i in range(NUM_ROBOTS):
            ...
            current_vel_local = current_state.twist.twist.linear
            world_vel_x = current_vel_local.x * math.cos(current_heading)
            world_vel_y = current_vel_local.x * math.sin(current_heading)
            current_vel_vec = np.array([world_vel_x, world_vel_y])
            ...
            for j in range(NUM_ROBOTS):
                ...
                dist = math.sqrt((current_pos.x - neighbor_pos.x)**2 + (current_pos.y - neighbor_pos.y)**2)
                if 0 < dist < NEIGHBOR_RADIUS:
                    ...
                    neighbor_q = neighbor_state.pose.pose.orientation
                    neighbor_heading = self.get_heading_from_quaternion(neighbor_q)
                    neighbor_vel_local = neighbor_state.twist.twist.linear
                    neighbor_world_vel_x = neighbor_vel_local.x * math.cos(neighbor_heading)
                    neighbor_world_vel_y = neighbor_vel_local.x * math.sin(neighbor_heading)
                    alignment_vec += np.array([neighbor_world_vel_x, neighbor_world_vel_y])
            ...
            if neighbor_count > 0:
                ...
                steer_wander = np.array([math.cos(new_heading), math.sin(new_heading)]) * WEIGHT_WANDER
            ...
            if dist_to_goal > ARRIVAL_RADIUS:
                ...
                steer_nav = vec_to_goal * (MAX_LINEAR_SPEED * (dist_to_goal / ARRIVAL_RADIUS))
            steer_nav = steer_nav * WEIGHT_NAVIGATION

            steer_obs = np.array([0.0, 0.0])
            closest_obs_dist = float('inf')
            closest_obs_vec = np.array([0.0, 0.0])
            is_avoiding = False
                ...
                for u in range(-check_radius_cells, check_radius_cells + 1):
                    for v in range(-check_radius_cells, check_radius_cells + 1):
                        ...
                        if self.is_obstacle(cell_x, cell_y):
                            ...
                            if 0 < dist_to_obs < OBSTACLE_SENSE_RADIUS:
                                if dist_to_obs < closest_obs_dist:
                                    closest_obs_dist = dist_to_obs
                                    closest_obs_vec = current_pos_vec - obs_pos_vec

            if closest_obs_dist < float('inf'):
                magnitude = 1.0 / (closest_obs_dist * closest_obs_dist)
                steer_obs = (closest_obs_vec / closest_obs_dist) * magnitude * WEIGHT_OBSTACLE
                is_avoiding = True

            if is_avoiding:
                final_linear_vec = (steer_obs + 
                                    separation_vec + 
                                    alignment_vec + 
                                    steer_nav * 0.1)
            else:
               final_linear_vec = (separation_vec + alignment_vec + cohesion_vec + steer_nav + steer_wander)
